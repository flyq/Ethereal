...
contract Mint is Verifier {
    ...
    event Deposit(
        Pairing.G1Point indexed soulbox,
        Pairing.G1Point commitment,
        uint256 timestamp
    );
    ...
    function register(
        ...
        require(
            verifySoulBox(_commitment, _proof, _soulBox),
            "Invalid  soul box proof"
        );
    ...
    function mint(Pairing.G1Point calldata _proof, uint256 _value) external {
        address sender = msg.sender;
        uint256 nonce = getNonce(sender);
        require(deposits[sender].registered, "User registered");
        require(
            verify(deposits[sender].commitment, _proof, nonce, _value),
            "Invalid mint proof"
        );
        ...
        uint256 soul = evaluateBarycentricPolynomial(
            0,
            deposits[sender].collectedY,
            wArray
        );
